...
export const updateOne = (Model) =>
    catchAsync(async(req, res, next) => {
        ...
        if (!UpdatedDoc) {
            ...
            return next(new AppError("No Product found with that ID", 404)); // return an error
        }
        res.status(200).json({
            status: "success",
            data: {
...
export const getOne = (Model) =>
    catchAsync(async(req, res, next) => {
        // const { id } = req.params;
        let query = Model.findById(req.params.id); // get a product from the collection 'Model' by id
        const doc = await query; // get a product from the collection 'Model' by id
        if (!doc) {
            return next(new AppError("No product found with that ID", 404));
        }
        res.status(200).json({
